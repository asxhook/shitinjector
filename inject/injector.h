...
void face_injecor_v2(LPCSTR window_class_name, LPCWSTR dll_path)
{
	...
	if (!dll_image)
		printf(xor_a("allah systems cant found dll\n"));
	...
	{		
		...
		cout << xor_a("allah_locate_base: 0x") << hex << allocate_base << endl;

		// fix reloc
		if (!relocate_image(allocate_base, dll_image, dll_nt_head))
		{
			driver().free_memory_ex(allocate_base);
			printf(xor_a("allah_relocate failed\n"));
		}

		printf(xor_a("allah_relocate image success!\n"));
		...
		printf(xor_a("allah_resolve import success!\n"));

		// write dll section's
		write_sections(allocate_base, dll_image, dll_nt_head);

		printf(xor_a("allocate success\n"));

		// call dll main
		call_dll_main(thread_id, allocate_base, dll_nt_head, false);

		printf(xor_a("allah_systems call dll > main.dll!\n"));

		// cleanup
		erase_discardable_sect(allocate_base, dll_nt_head);
		VirtualFree(dll_image, 0, MEM_RELEASE);

		printf(xor_a("allah systems cleaning drivers\n"));
		printf(xor_a("inject success have fun https://github.com/akeroyfix/ \n"));
